...
async def create_patient(db: aiosqlite.Connection, patient: PacienteCreate) -> PacienteResponse:
    ...
    cursor = await db.execute(
        """
        INSERT INTO patients (nome, cpf, data_nascimento, sexo)
        VALUES (?, ?, ?, ?)
        """,
    ...
    return PacienteResponse(id_paciente=patient_id, **patient.dict())

async def get_patient_by_id(db: aiosqlite.Connection, patient_id: int) -> Optional[PacienteResponse]:
    """Retrieves a patient by their ID."""
    cursor = await db.execute("SELECT * FROM patients WHERE id_paciente = ?", (patient_id,))
    row = await cursor.fetchone()
    if row:
        return PacienteResponse(**dict(row))
    return None

async def get_all_patients(db: aiosqlite.Connection, skip: int = 0, limit: int = 10) -> List[PacienteResponse]:
    """Retrieves all patients with pagination."""
    cursor = await db.execute("SELECT * FROM patients LIMIT ? OFFSET ?", (limit, skip))
...
async def update_patient(db: aiosqlite.Connection, patient_id: int, patient: PacienteUpdate) -> Optional[PacienteResponse]:
    ...
    cursor = await db.execute(f"UPDATE patients SET {fields} WHERE id_paciente = ?", tuple(values))
    await db.commit()
...
async def delete_patient(db: aiosqlite.Connection, patient_id: int) -> bool:
    ...
    cursor = await db.execute("DELETE FROM patients WHERE id_paciente = ?", (patient_id,))
    await db.commit()
